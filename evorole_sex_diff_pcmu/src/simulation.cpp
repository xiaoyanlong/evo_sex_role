...
namespace evorole {
  ...
  Individual Simulation::create_offspring(int day, Sex sex, int juv_stay, const Individual& female, const Individual& male)
  {
    ...
    auto day_mut_female = std::bernoulli_distribution(mut_par.m_pc1);
    if (day_mut_female(reng_)) og.pc1 = std::max(0, og.pc1 + (2 * binary(reng_) - 1));
    auto day_mut_male = std::bernoulli_distribution(mut_par.m_pc2);
    if (day_mut_male(reng_)) og.pc2 = std::max(0, og.pc2 + (2 * binary(reng_) - 1));
    return offspring;
  }
