...
namespace evorole {
  ...
  namespace {
    ...
    inline double add_p_tau_normal_mutation_if(double val, double P, double sigma, RENG& reng)
    {
        if (std::bernoulli_distribution(P)(reng)) {
            const auto s = std::normal_distribution<>(0.0, sigma)(reng);
            val = std::max(0.0, val + s);
        }
        return val;
    }
    ...
    template <typename RENGCARE>
    inline double add_care_normal_mutation_if(double val, double P, double sigma, RENGCARE& reng)
    {
        if (std::bernoulli_distribution(P)(reng)) {
            const auto s = std::normal_distribution<>(0.0, sigma)(reng);
            val = std::max(0.0, val + s);
  ...
  void Simulation::reproduction(int day)
  {
    ...
    for (size_t i = 0; i < last; ++i) {
     ...
     /* recorder_->record_mating(day, f[i]);
      ...
      const double pc1 = f[i].pool_stay = draw_care_days(f[i].genome.pc1, fmu); 
      const double pc2 = m[i].pool_stay = draw_care_days(m[i].genome.pc2, mmu + beta * m[i].genome.tau * m[i].genome.tau);
      ...
      // adults survive their time in the care dst if pool_stay >= std::floor(genome.pc1|2).
      (f[i].pool_stay >= std::floor(f[i].genome.pc1))
        ? pop_[Sex::female][Pool::care].push_back(f[i])
        : recorder_->record_death(day, Pool::care, f[i]);   // exact: day <- day + pc1
      (m[i].pool_stay >= std::floor(m[i].genome.pc2))
  ...
  Individual Simulation::create_offspring(int day, Sex sex, int juv_stay, const Individual& female, const Individual& male)
  {
    ...
    // mutation
    og.P = add_p_tau_normal_mutation_if(og.P, mut_par.m_P, mut_par.sigma_P, reng_);
    og.tau = add_p_tau_normal_mutation_if(og.tau, mut_par.m_tau, mut_par.sigma_tau, reng_);
    og.pc1 = add_care_normal_mutation_if(og.pc1, mut_par.m_pc, mut_par.sigma_pc, reng_);
    og.pc2 = add_care_normal_mutation_if(og.pc2, mut_par.m_pc, mut_par.sigma_pc, reng_);
    return offspring;
  }
  ...
  // draws the number of survived days given the per-day mortality mu in the caring state.
  int Simulation::draw_care_days(double max_days, double mu)
  {
    int real_max_day = std::bernoulli_distribution(max_days - std::floor(max_days))(reng_) ? static_cast <int> (std::ceil(max_days)) : static_cast <int> (std::floor(max_days));
    return std::min(real_max_day, std::geometric_distribution<int>(mu)(reng_));
  }


  // draws the number of survived days given the per-day mortality mu in other states.
